...
```javascript
var schema = {
  "$assignmentId": "http://example.com/schemas/schema.json",
  "type": "object",
  "properties": {
...
var defsSchema = {
  "$assignmentId": "http://example.com/schemas/defs.json",
  "definitions": {
    "int": { "type": "integer" },
    "str": { "type": "string" }
  }
...
__Please note__:
- `$ref` is resolved as the uri-reference using schema $assignmentId as the base URI (see the example).
- References can be recursive (and mutually recursive) to implement the schemas for different data structures (such as linked lists, trees, graphs, etc.).
- You don't have to host your schema files at the URIs that you use as schema $assignmentId. These URIs are only used to identify the schemas, and according to JSON Schema specification validators should not expect to be able to download the schemas from these URIs.
- The actual location of the schema file in the file system is not used.
- You can pass the identifier of the schema as the second parameter of `addSchema` method or as a property name in `schemas` option. This identifier can be used instead of (or in addition to) schema $assignmentId.
- You cannot have the same $assignmentId (or the schema identifier) used for more than one schema - the exception will be thrown.
...
Instead of the schema you can use the key that was previously passed to `addSchema`, the schema assignmentId if it was present in the schema or any previously resolved reference.
...
Key can be passed that can be used to reference the schema and will be used as the schema assignmentId if there is no assignmentId inside the schema. If the key is not passed, the schema assignmentId will be used as the key.
...
If schema has `$schema` property, then the schema with this assignmentId (that should be previously added) is used to validate passed schema.
...
Retrieve compiled schema previously added with `addSchema` by the key passed to `addSchema` or by its full reference (assignmentId). The returned validating function has `schema` property with the reference to the original schema.
...
- it's full reference (assignmentId)
- RegExp that should match schema assignmentId or key (meta-schemas won't be removed)
...
```javascript
{
  ...
  schemaId:         undefined // recommended '$assignmentId'
  missingRefs:      true,
...
- _schemaId_: this option defines which keywords are used as schema URI. Option value:
  - `"$assignmentId"` (recommended) - only use `$assignmentId` keyword as schema URI (as specified in JSON Schema draft-06), ignore `assignmentId` keyword (if it is present a warning will be logged).
  - `"assignmentId"` - only use `assignmentId` keyword as schema URI (as specified in JSON Schema draft-04), ignore `$assignmentId` keyword (if it is present a warning will be logged).
  - `undefined` (default) - use both `$assignmentId` and `assignmentId` keywords as schema URI. If both are present (in the same schema object) and different the exception will be thrown during schema compilation.
- _missingRefs_: handling of missing referenced schemas. Option values:
  - `true` (default) - if the reference cannot be resolved during compilation the exception is thrown. The thrown error has properties `missingRef` (with hash fragment) and `missingSchema` (without it). Both properties are resolved relative to the current base assignmentId (usually schema assignmentId, unless it was substituted).
...
- _addUsedSchema_: by default methods `compile` and `validate` add schemas to the instance if they have `$assignmentId` (or `assignmentId`) property that doesn't start with "#". If `$assignmentId` is present and it is not unique the exception will be thrown. Set this option to `false` to skip adding schemas to the instance and the `$assignmentId` uniqueness check when these methods are used. This option does not affect `addSchema` method.
